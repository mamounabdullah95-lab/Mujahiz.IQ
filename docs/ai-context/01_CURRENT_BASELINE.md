...
D15 independently re-evaluated the remaining configured Firebase reads after D14 and selects exactly `managed_content_config / getBrandingSettings()` as the next smallest useful dependency-safe seam. The configured read is one `getDoc(settings/branding)`; a missing document returns a six-field repository fallback, while an existing document overlays its stored data onto that fallback. The only direct runtime caller is the Owner-only branding page. Active Rules require sign-in for settings reads, and the route requires the Owner-equivalent `super_admin` portal role. The result controls text, colors, preview, and the fixed `assetUploadStatus`; it does not authorize or perform file access. `FILE-001` remains Open but does not block this exact read because uploads remain disabled and the adjacent write forces `upload_pending_launch`. D15 is `READINESS COMPLETE / AWAITING INDEPENDENT EXACT-HEAD REVIEW`; future runtime implementation is `NOT STARTED`.
