...
- **Verified current fact:** Current GitHub `main`: `3c66e52c011ec686c3a9b7d1dbe9db18a4800a07`, the merge of PR #116 after merged PRs #113, #114, and #115.
...
- **Verified current fact:** Current `main` tracks 24 local SQL migrations and 24 focused pgTAP files, with 24 physical PostgreSQL tables, 80 logical concepts, 37 Core Phase 1 concepts, 22 implemented and 15 unimplemented Core Phase 1 concepts, exactly one Claim RLS policy, and zero Claim mutation policies.
...
- **Verified current fact:** The authoritative logical PostgreSQL schema design and local migration slices are merged; 24 tracked local SQL migrations implement through the post-#118 bounded target-Supplier conflict resolver slice, while remaining business/application schema and runtime remain unimplemented.
...
The repository has completed the local privileged foundations: identity/profile/link structure, platform-role structure, role-backed administration-access structure, security-eligibility assessment structure, and the bounded current-principal relational resolver. Target-Supplier conflict remains a separate future predicate; Reviewer private reads, `assign_reviewer`, gateway/hosted authorization, and Production authority remain absent. Local relational eligibility is not Firebase authentication.

The next approved technical step is **Reviewer Private-Read Substrate — Readiness and Implementation**: Owner assignment queue, reviewer candidate eligibility/projection, assigned-reviewer queue/detail, and exact RLS/object-grant boundaries. It must explicitly investigate whether a private arbitrary-profile privileged-eligibility helper is required for candidate UUID evaluation; do not invent or expose that helper in this documentation sync. `supplier_claim.assign_reviewer` remains after the read substrate.
