...
Baseline ID: `baseline-2026-08-22-d21-supplier-favorites-firebase-adapter`
Updated: 2026-08-22
...
- **Verified current fact:** Current GitHub `main`: `7a3e2fe86fc2cc4e4f62855c6baf871343544415`, the manual merge of PR #165. Its first parent is D21/PR #164 merge `29c34d90aa902bd8df0b1977c92383905b99e9b5`; its second parent is independently reviewed D22 implementation head `1367efbc629c47cb7dc101e894185ce467ebbee3`. D20 is **COMPLETE / REVIEWED / MANUALLY MERGED**; its one Low remains bounded test-precision debt about direct binding of the actual `/contentPages/{pageId}` rule and singleton `super_admin` guarded-route nesting. The unchanged D16 formatting-sensitive production-wiring assertion remains a semantically correct, bounded non-blocking Nit. D21 is **COMPLETE / REVIEWED / MANUALLY MERGED** with two open/non-blocking Lows: duplicated date/sort helper maintenance debt and focused Date/invalid-string/Timestamp-like/absent-timestamp test precision. D22 is **COMPLETE / REVIEWED / MANUALLY MERGED**: independent exact-head review found 0 Critical, 0 High, 0 Medium, 2 Low, and 0 Nit; focused validation 33/33, full repository unit suite 274/274, `tsc -b --pretty false`, Vite production build, and `git diff --check` passed; PR Gate #276 / run `32596612349` succeeded. D23 is **IMPLEMENTATION COMPLETE ON BRANCH / AWAITING INDEPENDENT EXACT-HEAD REVIEW** on `codex/d23-next-firebase-read-seam`; it routes only `supplier_reviews / listSupplierReviews(supplierId, includePending)` through the existing D22 Firebase adapter, registry, and resolver, and closes D22 Low 1 with deterministic date/window/error tests and D22 Low 2 with `TimestampLike` typing. Firebase Production was not deployed, no Production/TEST data action occurred, no Rules/index/Auth/config or Provider manifest change occurred, no Supabase runtime capability was added, and Firebase remains authoritative for the complete current live feature set.
...
Historical Package C checkpoint (2026-08-18): Package B and C1–C5 were `COMPLETE`; PR #145 had merged the provider-contract kernel at `ea8e308f4b5276b04c3ecd644d7ecf27f5356a96`; and C6 post-merge Baseline/queue synchronization was the next control-plane task. That historical checkpoint was superseded when PR #146 merged C6. At that checkpoint, `main` was `f8dff27c69c05f567920f97e471dc4a06ed68c9b`, C6 and Package C were `COMPLETE`, and Package D was current. D1 was a Draft readiness PR only and did not authorize adapter implementation, service rewiring, Auth changes, provider switching, Supabase capability, data work, or deployment.
...
D7 started from clean `origin/main` `1c9b49ef74a92ccc652a65e30ef8b2f57bb44216` and defines the readiness contract in `75_SUPPLIER_DIRECTORY_FIREBASE_ADAPTER_READINESS.md`. Its independently reviewed head `b3aff41e5e7d9561dc4870f033f1d2f7ec390caa` passed with 0 Critical, 0 High, and 0 Medium findings; PR Gate #257 passed, and PR #150 was manually merged as `586a1922e71c91110177567b9a5e4ff96187d68c`. The selected read-only seam is exactly `listSuppliers()`, `listSuppliersPage(...)`, `listSupplierCandidates(...)`, and `getSupplier(...)` over the Firestore `suppliers` collection. It covers the approved full list, approved pagination, category candidates, and shared directory profile detail while excluding every Supplier write, submission, duplicate-index workflow, Claim, review, taxonomy, AI-intent, private-catalog, RFQ lifecycle/record, Auth, Rules/index, and provider-manifest change.
...
D10 is `COMPLETE / REVIEWED / MANUALLY MERGED`: implementation head `56c170c124ead1211bde0d318c4e61d15dafd79e`, independent review 0 Critical / 0 High / 0 Medium / 0 Low / 0 Nit, PR Gate #262 / run `32334282427` success, and PR #153 manually merged as current GitHub `main` `44f850d485a83b48091052046519668731da0f37`. GitHub `main` contains the Firebase-only adapter for configured `listMaterialTerms()` behind the existing `supplier_taxonomy_dictionary` Provider Contract feature. Firebase Production was not deployed, no Production/TEST data action occurred, no Supabase runtime capability was added, and Firebase remains authoritative for the complete taxonomy/dictionary feature.

D11 readiness selects exactly `listRegistrationSectors()` as the next smallest dependency-safe configured Firebase read seam under the existing `supplier_taxonomy_dictionary` feature. The current configured read is one `getDoc` at `publicConfig/registration`; it accepts only an array-valued `sectors` field, filters to active entries, sorts ascending by numeric subtraction on `order`, and returns the 17 active ordered repository defaults when the document/array is absent, no active configured entries remain, or the configured Firebase read throws. Demo/local reads the `mujahiz-iq-workspace:registrationSectors` local array, applies the same active filter/order, and uses the same defaults when empty or unreadable. Direct callers are `RegisterPage`, `CompleteProfilePage`, and the Owner registration-sectors administration page. Active `firestore.rbac.rules` makes `/publicConfig/{configId}` publicly readable and Owner-writable. This is readiness only: no runtime adapter, manifest, Rules/index, Auth, SQL/RLS, Supabase, data, hosted, or deployment change is authorized. The seven Open gates remain exactly `ORG-001`, `ORG-002`, `MSG-002`, `FILE-001`, `BILL-001`, `RES-001`, and `MIG-002`.

D11 is now `COMPLETE / REVIEWED / MANUALLY MERGED`: readiness head `d5454560907436527789786996d59a1fd6374102`, independent review 0 Critical / 0 High / 0 Medium / 1 Low / 0 Nit, PR Gate #263 / run `32336621145` success, and PR #154 manually merged as current GitHub `main` `f51c7b123dfa2643623fbd2d5a26ccfecfd33c82`. The Low finding requires one shared taxonomy Firebase implementation, registry, and resolver path.

D12 is `COMPLETE / REVIEWED / MANUALLY MERGED`: implementation head `cc4a5a8a3653478d201b691a74c72e51a1a16b13`, independent exact-head review 0 Critical / 0 High / 0 Medium / 0 Low / 0 Nit, PR Gate #264 / run `32353894728` success, and PR #155 manually merged as current GitHub `main` `4e0867e37b353e5b22e4451f606b964013faba48`. Its final taxonomy architecture is `supplier_taxonomy_dictionary` -> one shared registry -> one Firebase implementation instance -> one resolver -> `listMaterialTerms()` and `listRegistrationSectors()`, fully resolving D11's prior Low finding. Demo/local remains before provider resolution, Provider failures fail closed, the registration-sector write remains inline and unchanged, Firebase Production was not deployed, no Production/TEST data action occurred, and no Supabase runtime capability was added.

D13 independently re-evaluated the remaining configured Firebase read seams from the current 17-feature Provider Contract vocabulary and uniquely selected `managed_content_config / getPublishedContentPage(slug)` as the next smallest dependency-safe seam. The configured path queries `contentPages` with exact equality filters `slug == <caller slug>` and `status == "published"`, applies `limit(1)`, returns `null` when empty, and maps the first document as `{ id: snapshot.id, ...snapshot.data() }` with no explicit ordering, cache, retry, pagination, or realtime subscription. Its only direct caller, `PublicContentPage`, catches errors and substitutes repository static public content. Active `firestore.rbac.rules` permits a content-page read only when the returned record is published or the caller is Owner; the selected public query includes the required published constraint and needs no Rules/Auth/index change. Demo/local remains an explicit pre-resolution localStorage lookup. D13 is `COMPLETE / REVIEWED / MANUALLY MERGED`: readiness head `6617875f137de3b2c9f3610a3e80be0e9ace6554`, independent exact-head review 0 Critical / 0 High / 0 Medium / 0 Low / 0 Nit, static assertions 55/55, Markdown links 32/32, focused provider/runtime/Demo tests 18/18, and `git diff --check` passed. PR Gate #265 / run `32412107538` succeeded; PR #156 was manually merged as GitHub `main` `2144c9c6959d0594de3f79005f72041cf7fa219c`.

D14 is `COMPLETE / REVIEWED / MANUALLY MERGED`: implementation head `64f4ba52c639e4f8afef35ecfcbc7f3733a217c0`, independent exact-head review 0 Critical / 0 High / 0 Medium / 1 Low / 0 Nit, focused tests 73/73, full repository unit suite 242/242, `tsc -b` passed, Vite production build passed, and `git diff --check` passed. PR Gate #266 / run `32521086096` succeeded; PR #157 was manually merged as GitHub `main` `5736d6142cce47ac51ecf247e87d535481234bf21`. Final architecture is `managed_content_config` -> one Firebase implementation instance -> one feature registry -> one resolver -> `getPublishedContentPage(slug)`. Demo/local remains before Provider resolution; Firebase/query/mapping failures propagate from the service, while `PublicContentPage` retains its caller-level repository-static presentation fallback. Its historical Low is **CLOSED BY D16 TEST COVERAGE**: the D16 test explicitly binds `SHIPPED_PROVIDER_MANIFEST`, feature `managed_content_config`, and the one real `managedContentConfigImplementations` registry in the production resolver call. No deployment, Production/TEST data action, Rules/index/Auth/config change, Provider manifest change, or Supabase runtime capability occurred.

D15 independently re-evaluated the remaining configured Firebase reads after D14 and selected exactly `managed_content_config / getBrandingSettings()` as the next smallest useful dependency-safe seam. The configured read is one `getDoc(settings/branding)`; a missing document returns a six-field repository fallback, while an existing document overlays its stored data onto that fallback. Branding and Admin Operations are both **Low**-complexity one-document candidates. Branding remains selected after a runtime-value and audience-boundary tie-breaker: its Owner-only page visibly uses Branding text and colors in editing and preview, whereas Admin Operations only populates its broader Admin/Owner settings form and has no verified downstream runtime consumers. Active Rules require sign-in for settings reads, and the Branding route requires the Owner-equivalent `super_admin` portal role. The read loads and preserves `assetUploadStatus`, but `OwnerBrandingPage` does not render or consult that field; `DisabledFileUpload` is independently inert. `FILE-001` remains Open but does not block this exact read because uploads remain disabled and the adjacent write forces `upload_pending_launch`. D15 is `COMPLETE / REVIEWED / MANUALLY MERGED`: corrected readiness head `494efe40c173dc66fcef71674a04e3a91818cc1c`, fresh independent review 0 Critical / 0 High / 0 Medium / 1 Low / 0 Nit, PR Gate #268 / run `32526221532` succeeded, and PR #158 was manually merged as GitHub `main` `006a5ea4561360a60af471c548dc9fabd16ef6a5`. Its Low was documentation-only: stale wording said one documentation commit although PR #158 had two commits after correction. D16 is `COMPLETE / REVIEWED / MANUALLY MERGED`.

D16 is `COMPLETE / REVIEWED / MANUALLY MERGED`: implementation head `87996359e5b42f5ebf8e07e6bc21463d3ba0d306`, independent exact-head review 0 Critical / 0 High / 0 Medium / 1 Low / 1 Nit, focused tests 34/34, full repository unit suite 245/245, `tsc -b` passed, Vite production build passed, and `git diff --check` passed. PR Gate #269 / run `32558505576` succeeded; PR #159 was manually merged as GitHub `main` `604dcdae8cf4f23ab69c9074bd4ca7d931b5717f`. D16 extends the existing single `managed_content_config` Firebase adapter implementation, registry, and resolver with `getBrandingSettings()`, preserving Demo/local before resolution and Firebase aggregate authority. It closes D14's historical production-wiring Low with exact binding of `SHIPPED_PROVIDER_MANIFEST`, feature `managed_content_config`, and `managedContentConfigImplementations`; that Low must not be reopened. The one D16 Nit is limited test-maintainability debt: the semantically correct assertion is somewhat formatting-sensitive. No Firebase deployment, Production/TEST data action, Rules/index/Auth/config or manifest change, or Supabase runtime capability occurred.
