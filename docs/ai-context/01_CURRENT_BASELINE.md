...
- **Verified current fact:** Current GitHub `main`: `ecf60e756ffba5f04a79fc71dcbcdf6c60bf7004`, the merge of PR #114 after merged PR #113.
...
- **Verified current fact:** Current `main` tracks 23 local SQL migrations and 23 focused pgTAP files, with 24 physical PostgreSQL tables, 80 logical concepts, 37 Core Phase 1 concepts, 22 implemented and 15 unimplemented Core Phase 1 concepts, exactly one Claim RLS policy, and zero Claim mutation policies.
...
- **Verified current fact:** The authoritative logical PostgreSQL schema design and local migration slices are merged; 23 tracked local SQL migrations implement through the post-#116 bounded privileged-actor resolver slice, while remaining business/application schema and runtime remain unimplemented.
