...
Baseline ID: `baseline-2026-08-09-rel-foundation-claim-structure-approved`
Updated: 2026-08-09
...
- **Verified current fact:** Current GitHub `main`: `8dcf556aea1460dd4ed9510854a644225ee0ad3a`, the merge of PR #95; Draft PR #96 is synchronized from this commit by a normal merge.
...
- **Verified current fact:** PR #95 is merged as current `main` commit `8dcf556aea1460dd4ed9510854a644225ee0ad3a`; it added exactly the empty, fully revoked, local-only `internal.idempotency_keys` and `internal.domain_events` foundation plus focused synthetic pgTAP as the fourteenth local SQL slice. It added no real rows, trusted-command/event worker runtime, notification runtime, RLS/Auth bridge, hosted operation, Firebase access, Production/TEST data behavior, migration, or deployment.
- **Verified current fact:** The fourth through fourteenth SQL slices remain local-only and synthetic-data-only; no RLS, Auth bridge, hosted Supabase operation, Firebase access, Production/TEST data operation, data migration, seed, backfill, deployment, or remote SQL occurred.
...
- **Verified current fact:** GitHub `main` contains fifteen local identity/business/application/audit/reliability tables: `public.user_profiles`, `public.platform_role_assignments`, `public.supplier_profiles`, `public.categories`, `public.administrative_areas`, `public.supplier_locations`, `public.supplier_category_assignments`, `public.supplier_capabilities`, `public.supplier_payment_options`, `public.supplier_contacts`, `public.supplier_ownerships`, non-exposed `internal.identity_provider_links`, `internal.audit_logs`, `internal.idempotency_keys`, and `internal.domain_events`. No real platform-role, audit, idempotency, or event rows, RLS, Auth bridge, Supabase Auth users, Storage buckets, Edge Functions, `supabase-js` frontend integration, or Migration Engine runtime exists.
...
- **Verified current fact:** Firebase Production remains unchanged and authoritative for the live application. No Firebase or Production data was migrated, exported, seeded, backfilled, or changed by PR #47, PR #48, PR #49, PR #51, PR #54, PR #59, PR #61, PR #62, PR #64, PR #65, PR #66, PR #67, PR #68, PR #69, PR #70, PR #71, PR #72, PR #73, PR #74, PR #75, PR #76, PR #77, PR #78, PR #79, PR #80, PR #81, PR #82, PR #83, PR #85, PR #86, PR #87, PR #88, PR #89, PR #90, PR #91, PR #93, PR #94, or PR #95.
...
- **Verified current fact:** The authoritative logical PostgreSQL schema design and its first local migration-control/traceability slice are merged; 14 tracked local SQL migrations implement the second local identity, third local Supplier-profile, fourth local category, fifth local administrative-area, sixth local Supplier-location, seventh local Supplier-category assignment, eighth local Supplier-capability, ninth local Supplier-payment-option, tenth local Supplier-contact, eleventh local Supplier-ownership, twelfth local platform-role-assignment, thirteenth local audit-log, and fourteenth local REL-foundation slices, while the remaining business/application schema remains unimplemented.
...
- **Verified current fact:** Of the 36 Core Phase 1 concepts, 19 are implemented locally and 17 remain deferred. The first 4 logical concepts use 6 physical tables because `migration_record_mappings` is decomposed across 3 relations; the local SQL foundation has 21 physical tables.
...
- **Verified current fact:** ID-001 is Resolved for the approved hybrid identity-authority and privileged-actor contract, AUD-001 is Resolved for the approved audit-evidence and trusted-mutation contract, SEARCH-001 is Resolved for the owner-approved PostgreSQL Option A technology contract, RFQ-003 is Resolved for the owner-approved Option B commercial semantics contract, MSG-003 is Resolved for the owner-approved Option C notification contract, and REL-001's coherent two-table foundation is implemented locally. No search, RFQ/quotation, notification, Claim, event-processing, or idempotency runtime is implemented. Exactly 7 approval gates remain Open.
...
**REL foundation implemented locally; Claim Option A selected but not implemented; no Claim or notification runtime exists**

Merged PR #95 implemented exactly the approved empty, fully revoked, local-only `internal.idempotency_keys` and `internal.domain_events` foundation plus focused synthetic pgTAP as the fourteenth slice. Current `main` therefore contains 14 tracked local SQL migrations and 21 physical tables representing 19 implemented Core Phase 1 concepts; 17 remain deferred. On 9 August 2026, the Technical/Data Owner approved [`37_SEARCH_001_POSTGRESQL_SEARCH_TECHNOLOGY_CONTRACT_REVIEW.md`](../supabase-migration/37_SEARCH_001_POSTGRESQL_SEARCH_TECHNOLOGY_CONTRACT_REVIEW.md) Option A: relational filters plus bilingual PostgreSQL FTS first, with no initial `pg_trgm` and no external Phase 1 search service. SEARCH-001 is Resolved for architecture only; no search SQL, extension, index, projection, RPC, RLS, frontend, AI, hosted, data, or runtime work is selected or authorized.
...
On 9 August 2026, the Product/Data/Security Owner approved [`41_CLAIM_SUPPLIER_PROFILE_STRUCTURAL_AND_COMMAND_READINESS_REVIEW.md`](../supabase-migration/41_CLAIM_SUPPLIER_PROFILE_STRUCTURAL_AND_COMMAND_READINESS_REVIEW.md) Option A: exactly one future empty, fully revoked, local-only `public.supplier_ownership_claims` table as the next Claim-specific slice after REL. Claim v1 uses one durable write-once assigned reviewer with no reassignment or Owner override; evidence remains a bounded private aggregate on the Claim; and no separate evidence, reviewer, history/event, idempotency, rate-limit, attachment, file, or notification table is selected. The Claim table is not implemented by PR #96; if implemented next, it will be the fifteenth tracked local migration. FILE-001 and the other six Open gates remain unchanged.
