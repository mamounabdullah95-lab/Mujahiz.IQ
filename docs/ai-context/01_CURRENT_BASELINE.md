...
Baseline ID: `baseline-2026-08-10-privileged-access-security-contract-approved`
Updated: 2026-08-10
...
- **Verified current fact:** Current GitHub `main`: `6ea8342a502b4ee38b01ff49785e56c70a00e6b4`, the merge of PR #111 after merged PR #110.
...
- **Owner-approved contract, unimplemented:** On 10 August 2026, the Product/Security/Data Owner approved document 55 in Draft PR #112: Access Option A using `public.access_grants` only for `platform_administration`, and Security Option A using separate restricted `internal.security_eligibility_assessments`. Exactly two future empty local structural slices are selected, access first and security second. Approval creates no table, row, resolver, command, bootstrap, hosted authority, or Production change.
...
- **Verified current fact:** The authoritative logical PostgreSQL schema design and local migration slices are merged; 20 tracked local SQL migrations implement through the twentieth local Claim withdraw slice, while remaining business/application schema and runtime remain unimplemented.
...
- **Verified implementation fact:** Of the 37 Core Phase 1 concepts, 20 are implemented locally and 17 remain unimplemented. The first 4 logical concepts use 6 physical tables because `migration_record_mappings` is decomposed across 3 relations; the local SQL foundation remains unchanged at 22 physical tables, 20 migrations, and 20 pgTAP files.
