...
Baseline ID: `baseline-2026-08-10-claim-submit-gateway-reviewer-readiness-synchronized`
Updated: 2026-08-10
...
- **Verified current fact:** Current GitHub `main`: `52b2043775d74374595c4cfbb12e16aaef465d10`, the merge of PR #105 after merged PR #103 and PR #104.
...
- **Verified current fact:** PR #102 remains the seventeenth local-only Claimant self-read slice. PR #103 is merged as the eighteenth local-only migration implementing only `supplier_claim.submit`; PR #104 is merged gateway/HMAC/pool readiness documentation only; PR #105 is merged Reviewer readiness documentation only. No mutation RLS policy, hosted gateway, signed-token staging proof, Firebase change, or Production/TEST operation exists.
...
- **Verified current fact:** Current `main` tracks 18 local SQL migrations and 18 focused pgTAP files with **1,181 planned assertions**. PR #103 exact-head validation applied all 18 migrations and passed all **1,181/1,181** assertions in disposable local PostgreSQL; this is local synthetic evidence, not hosted or Production evidence.
...
- **Verified current fact:** The authoritative logical PostgreSQL schema design and local migration slices are merged; 18 tracked local SQL migrations implement through the eighteenth local `supplier_claim.submit` foundation slice, while remaining business/application schema and runtime remain unimplemented.
...
**Claimant self-read RLS/projection and `supplier_claim.submit` implemented locally; gateway/reviewer readiness remains design-only**

Merged PR #99 implemented the empty Claim foundation; PR #101 added identity context; PR #102 added Claim FORCE RLS, exactly one claimant self-select policy, and one minimized claimant projection; and PR #103 added the eighteenth local-only `supplier_claim.submit` slice. Current `main` contains 18 migrations, 18 pgTAP files, 22 physical tables, 20 implemented Core Phase 1 concepts, and 16 deferred concepts; 1,181/1,181 exact-head assertions passed. Claim self-read RLS remains one policy and no mutation RLS policy exists. The five other Claim commands remain unimplemented. PR #104 and PR #105 are documentation/design only; no hosted gateway or signed-token staging proof exists.
