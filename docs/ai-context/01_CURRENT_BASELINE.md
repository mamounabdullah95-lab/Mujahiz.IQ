...
- **Verified current fact:** Current GitHub `main`: `85a68c44e01d8d3bf86bdbf4d71f8092abfb4f3a`, the merge of PR #118 after merged PRs #113–#119.
...
- **Verified current fact:** PR #110 adds no table, Claim mutation RLS policy, notification, hosted Supabase capability, Firebase integration, real row, seed, backfill, remote migration, or deployment. Current local structure has exactly three Claim SELECT RLS policies, zero Claim mutation RLS policies, and both Claim mutation commands use durable Phase-A reservation plus separately fenced Phase-B execution.
...
- **Verified current fact:** Current `main` tracks 25 local SQL migrations and 24 focused pgTAP files, with 24 physical PostgreSQL tables, 80 logical concepts, 37 Core Phase 1 concepts, 22 implemented and 15 unimplemented Core Phase 1 concepts, exactly three Claim SELECT RLS policies, and zero Claim mutation policies.
...
- **Verified current fact:** The authoritative logical PostgreSQL schema design and local migration slices are merged; 25 tracked local SQL migrations implement through the post-#120 Reviewer Private-Read Substrate slice, while remaining business/application schema and runtime remain unimplemented.
...
The repository has completed the local privileged foundations: identity/profile/link structure, platform-role structure, role-backed administration-access structure, security-eligibility assessment structure, `claim_security.current_privileged_actor_v1()`, and the bounded `claim_security.target_supplier_conflict_v1(uuid, uuid, uuid)` resolver. Reviewer Private-Read Substrate includes the Owner assignment queue, reviewer candidate projection/eligibility delivery, assigned-reviewer queue/detail, and exact RLS/object grants. `supplier_claim.assign_reviewer`, approve/reject/expire, access/security administration, gateway/hosted authorization, and Production authority remain absent. Local relational eligibility is not Firebase authentication; local conflict `clear` is not Reviewer authorization.

The next dependency-safe technical step is **`supplier_claim.assign_reviewer` — Trusted Command Readiness and Implementation**. The future command must repeat all authorization and candidate checks after deterministic locks and preserve write-once assignment with `submitted -> under_review`; no reassignment, Owner override, or Admin assignment is permitted.
