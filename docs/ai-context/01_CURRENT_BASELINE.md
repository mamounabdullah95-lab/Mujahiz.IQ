...
Baseline ID: `baseline-2026-08-09-claim-runtime-context-command-contract-approved`
Updated: 2026-08-09
...
- **Verified current fact:** Current GitHub `main`: `d910bf43ea4fc5981da5869ed298b986a1bdb4bd`, the merge of PR #102.
...
- **Verified current fact:** PR #102 is merged as current main commit d910bf43ea4fc5981da5869ed298b986a1bdb4bd; it added the seventeenth local-only migration with Claim FORCE RLS, exactly one claimant self-select policy, and one minimized SECURITY INVOKER claimant status/history projection. It added no Claim mutation policy, trusted command, browser/API grant, reviewer/Admin/Owner access, gateway, hosted operation, Firebase/Production/TEST access, data movement, or deployment.
- **Verified current fact:** The fourth through seventeenth SQL slices remain local-only and synthetic-data-only. PR #102 is the sole read-only Claim RLS slice: it enables Claim FORCE RLS, adds exactly one claimant self-select policy, and adds one minimized claimant projection; no Claim mutation policy, complete Auth bridge, hosted Supabase operation, Firebase access, Production/TEST data operation, data migration, seed, backfill, deployment, or remote SQL occurred.
...
- **Verified current fact:** Current `main` tracks 17 local SQL migrations and 17 focused pgTAP files with **1,068 planned assertions**. PR #102 exact-head validation applied all 17 migrations and passed all **1,068/1,068** assertions in disposable local PostgreSQL; this is local synthetic evidence, not hosted or Production evidence.
...
- **Verified current fact:** The authoritative logical PostgreSQL schema design and its first local migration-control/traceability slice are merged; 17 tracked local SQL migrations implement the second local identity, third local Supplier-profile, fourth local category, fifth local administrative-area, sixth local Supplier-location, seventh local Supplier-category assignment, eighth local Supplier-capability, ninth local Supplier-payment-option, tenth local Supplier-contact, eleventh local Supplier-ownership, twelfth local platform-role-assignment, thirteenth local audit-log, fourteenth local REL-foundation, fifteenth local Claim-foundation, sixteenth local Claim runtime identity-context foundation, and seventeenth local Claimant self-read RLS/projection foundation slices, while the remaining business/application schema and runtime remain unimplemented.
...
- **Verified current fact:** Of the 36 Core Phase 1 concepts, 20 are implemented locally and 16 remain deferred. The first 4 logical concepts use 6 physical tables because `migration_record_mappings` is decomposed across 3 relations; the local SQL foundation has 22 physical tables.
...
**Claimant self-read RLS/projection foundation implemented locally; trusted-command architecture approved; zero Claim mutation commands exist**

Merged PR #99 implemented exactly the approved empty, fully revoked, local-only `public.supplier_ownership_claims` foundation plus focused pgTAP as the fifteenth slice. Merged PR #101 added the local-only `mujahiz_claim_runtime` role and non-exposed `claim_security` transaction-local principal helper as the sixteenth slice. Merged PR #102 added Claim FORCE RLS, exactly one claimant self-select policy, and one minimized `SECURITY INVOKER` claimant projection as the seventeenth slice. Current `main` therefore contains 17 tracked local SQL migrations and 17 pgTAP files, 22 physical tables representing 20 implemented Core Phase 1 concepts, and 16 deferred concepts; PR #102 exact-head validation passed 1,068/1,068 assertions. The claimant self-read substrate exists, but zero trusted Claim mutation commands exist.
