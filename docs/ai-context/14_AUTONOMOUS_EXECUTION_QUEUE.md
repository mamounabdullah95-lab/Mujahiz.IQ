...
- Validation: focused adapter/Provider/runtime/Demo/route tests 33/33; full unit suite 274/274; `tsc -b --pretty false` and Vite production build passed. The initial sandboxed full-suite and Vite runs encountered the known esbuild directory restriction; one approved unrestricted rerun of each passed without code changes. One bounded correction loop changed the adapter to receive the pre-existing shared date converter through its Provider composition, retaining exact sorting behavior while avoiding a second local helper.
