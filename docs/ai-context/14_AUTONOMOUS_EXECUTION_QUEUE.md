...
- State: `COMPLETE`.
- Evidence: PR #144 merged; approved head `e3520d5ae1c1cacedf3db60f46c6df5cf99cebe7`; merge commit `022fbc5b44377542c9e0e909eca0b32c5a93e889`; Gate #246 succeeded.
...
- State: `COMPLETE`.
- Evidence: final exact-head independent review reported 0 Critical / High / Medium / Low findings, all prior findings closed, and `APPROVE FOR MANUAL MERGE`.
...
- State: `COMPLETE`.
- Evidence: Owner manually merged PR #144; latest `origin/main` reverified at `022fbc5b44377542c9e0e909eca0b32c5a93e889`.
...
- Final lifecycle: implementation head `80a1db1b59f50ef1e0a6a2bba55b2834b2e410ea` received independent exact-head review 0 Critical / 0 High / 0 Medium / 2 Low / 0 Nit; independent focused validation 54/54; full unit suite 265/265; `tsc -b --pretty false`, Vite production build, and `git diff --check` passed; PR Gate #275 / run `32594492895` succeeded; PR #164 was manually merged as GitHub `main` `29c34d90aa902bd8df0b1977c92383905b99e9b5`. D21 Low 1 (duplicated date/sort helper) and Low 2 (focused Date/invalid-string/Timestamp-like/absent-timestamp precision) remain open and non-blocking because D22 does not naturally touch that adapter or harness.
...
### D22 - Buyer self-review Firebase Provider read implementation

- State: `COMPLETE / REVIEWED / MANUALLY MERGED`.
- Base: GitHub `main` `29c34d90aa902bd8df0b1977c92383905b99e9b5` (manual merge of PR #164).
- Selection: exactly `supplier_reviews / listMyReviews(userId)`. It is active at one Buyer-only, allow-pending route and receives the existing authoritative `firebaseUser.uid` from `MyReviewsPage`. It is a smaller one-audience seam than the other review operations: public/active-Buyer supplier reviews, Owner/Supplier profile visibility, and Admin moderation remain separate inline operations. `getPlatformSettings()` remains deferred for broad cross-aggregate settings/write coupling; feedback retains Buyer/Admin audience and adjacent moderation scope; operational reporting and audit evidence retain cache/cross-aggregate or sensitivity concerns; AI intent remains an external Firebase AI capability, not a datastore read.
- Scope: add one `SupplierReviewsImplementation`, one Firebase adapter instance, one feature registry, and one resolver; route only configured `listMyReviews(userId)` through it. Preserve Demo/local first (`demoListMyReviews` filters only), exactly one configured `reviews` query with `reviewedBy == userId`, no Firestore order/limit/cursor, stored-ID overwrite mapping, client newest-first `createdAt` sorting after the unbounded Firebase read, client `slice(0, 100)`, empty `[]`, and unchanged error propagation.
- Security and boundaries: the sole caller remains under `<RoleProtectedRoute allowedRoles={["buyer"]} allowPending />`; the exact user ID is unchanged. The existing Rule allows Buyer self-review reads by `resource.data.reviewedBy == request.auth.uid`; the existing `reviews` `reviewedBy`/`createdAt` index remains unmodified, and the selected query adds no ordering or index requirement. `submitSupplierReview`, `moderateReview`, all other review reads, and all feedback operations remain inline and unchanged. Firebase remains authoritative; shipped `supplier_reviews` authority remains Firebase; Demo/local remains before the resolver; synthetic Supabase selection fails closed. No Rules/index/Auth/configuration, manifest/vocabulary, Supabase, SQL/RLS, migration, Production/TEST data, deployment, fallback, probing, dual read, or dual write is included. The seven Open gates remain unchanged.
- Final lifecycle: implementation head `1367efbc629c47cb7dc101e894185ce467ebbee3` received independent exact-head review 0 Critical / 0 High / 0 Medium / 2 Low / 0 Nit; focused validation 33/33; full unit suite 274/274; `tsc -b --pretty false`, Vite production build, and `git diff --check` passed; PR Gate #276 / run `32596612349` succeeded; PR #165 was manually merged as GitHub `main` `7a3e2fe86fc2cc4e4f62855c6baf871343544415`. D22 Low 1 (missing direct Timestamp-like/missing/throwing-converter/>100 retained-window tests) and Low 2 (`never` converter typing) are closed by D23 because it extends the same adapter and harness without changing D22 runtime behavior. One bounded correction loop changed the adapter to receive the pre-existing shared date converter through its Provider composition, retaining exact sorting behavior while avoiding a second local helper.

### D23 - public supplier-review Firebase Provider read implementation

- State: `IMPLEMENTATION COMPLETE ON BRANCH / AWAITING INDEPENDENT EXACT-HEAD REVIEW`.
- Base: GitHub `main` `7a3e2fe86fc2cc4e4f62855c6baf871343544415` (manual merge of PR #165).
- Selection: exactly `supplier_reviews / listSupplierReviews(supplierId, includePending)`. It is the remaining active, bounded read in the already-composed aggregate. Its sole direct runtime caller is `SupplierProfilePage`, under the active Buyer supplier-profile route, and invokes the default approved-only form. The public route does not pass or translate an identity. The retained optional `includePending` branch is part of the existing exported contract and has no direct caller.
- Scope: extend the existing D22 `SupplierReviewsImplementation`, Firebase adapter, single registry, and single resolver; route only the configured facade branch through it. Preserve Demo/local first (filter only; no configured sort/cap), exactly one `reviews` query per invocation, `supplierId == supplierId` plus `status == approved` only when `includePending` is false, no Firestore order/limit/cursor, stored-ID overwrite mapping, post-read newest-first `createdAt` sorting, client `slice(0, 50)`, empty `[]`, and unchanged query/read/mapping/date errors. Existing D22 `listMyReviews` behavior remains unchanged.
- Security and boundaries: the default caller is within `<RoleProtectedRoute allowedRoles={buyerRoles} requireAccess />`; the unchanged Rule permits active Buyers to read approved reviews. The current `reviews` supplier/status/createdAt composite is unchanged; D23 adds no order or index requirement. `submitSupplierReview`, `moderateReview`, `listPendingReviews`, and all feedback operations remain inline and unchanged. Firebase remains authoritative; shipped `supplier_reviews` authority remains Firebase; Demo/local remains before the resolver; synthetic Supabase selection fails closed. No Rules/index/Auth/configuration, manifest/vocabulary, Supabase, SQL/RLS, migration, Production/TEST data, deployment, fallback, probing, dual read, or dual write is included. The seven Open gates remain unchanged.
- Validation: focused supplier-reviews/Provider Contract/Firebase runtime/Demo/Rules/route checks 38/38; full repository unit suite 275/275; `tsc -b --pretty false`, Vite production build, and `git diff --check` passed. The first sandboxed full-suite and Vite attempts hit the known esbuild directory-access restriction; one approved unrestricted rerun of each passed without code changes. One bounded correction loop fixed the synthetic throwing-date test so sorting invokes the supplied converter.
