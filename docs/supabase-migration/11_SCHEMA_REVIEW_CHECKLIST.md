...
### Post-PR #75 capability evidence and payment-options-only contract review (not part of the 119 design-review items)
...
- [x] Verified refreshed `origin/main` `25536e09d84adac950023e5903c855cbf847b236`, merged PR #75 lineage, 14 physical tables, 12 implemented / 24 deferred Core Phase 1 concepts, and 12 unchanged Open gates.
- [x] Payment options are indicative Supplier-profile assertions only; RFQs, quotations, purchase orders, and contracts independently own transaction terms and always take precedence.
...
- [x] Credit availability, explicit `credit_not_offered`, absence-as-unknown, 1-365 calendar-day duration, positive/negative mutual exclusion, and four exact start events are explicit; no current Firebase start value maps automatically.
- [x] Advance payment is a non-default timing option with a nullable reviewed percentage; zero, invented 100%, mandatory/refundable/milestone meaning, and RFQ-derived evidence are excluded.
...
- [x] Product/Data ownership, commercial review authority, source/mapping provenance, temporal lifecycle, validity, material-change supersession, and no-normal-hard-delete behavior are explicit.
...
- [x] The base relation remains revoked/non-public; no anonymous projection is recommended; any later Buyer projection is field-minimized, indicative, and separately approved under SEC-001.
- [x] The pre-migration package requires bounded manifests, approved vocabularies/mappings, collision/exception reports, deterministic traces, dry-run reconciliation, rollback, freshness policy, projection tests, and explicit environment/data authority.
- [x] One typed table is recommended over partial, four-table, parent/child, JSONB/profile, capability, RFQ, quotation, or billing boundaries as the smallest dependency-safe future slice.
- [ ] Product/Data Owner approval is recorded for the payment contract, code sets, exact credit/advance meanings, note/projection boundary, freshness policy, reviewer delegation, and ninth-slice direction.
