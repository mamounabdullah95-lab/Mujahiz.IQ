...
| --- | --- | --- | --- | --- | --- | --- |
| `supplier_taxonomy_dictionary` | `listMaterialTerms()` -> `DirectoryPage`, `AdminMaterialDictionaryPage`; `listTermSuggestions(...)` -> `AdminMaterialDictionaryPage`, `AdminDashboardPage`, and Demo/local portal metrics; `listRegistrationSectors()` -> registration/profile flows | Active material terms: Low. Suggestions: Moderate because examples may contain user query text and creator IDs. Registration sectors have a distinct path/access and fallback contract. | Suggestions are captured and moderated by `recordTermSuggestions`, `approveTermSuggestion`, and `ignoreTermSuggestion`; approval also writes audit evidence. Registration-sector reads remain a separate deferred boundary. | No Open gate directly blocks the active-term read. | **Eligible at Low risk only for `listMaterialTerms()`** | **Selected.** It is the smallest functional, low-sensitivity read seam. Admin suggestion reads and registration-sector reads remain excluded. |
...
- When multiple active custom terms have the same mapped ID, each `Map.set(term.id, term)` is applied in input order: the later value wins, while replacing the existing key does not move its established insertion position.
...
Firebase remains authoritative for the whole `supplier_taxonomy_dictionary` feature, including material terms, term suggestions, suggestion moderation, registration sectors, current categories/taxonomy, and their command effects.
...
| Duplicate custom IDs | Later duplicate mapped IDs replace earlier values without moving the key's established insertion position | Deterministic unit/injected test using the real merge helper |
