...
The serious finalists were: (1) `supplier_reviews / listSupplierReviews`, selected because the aggregate's Firebase authority, existing composition, caller, Rule, collection path, mapping, and query shape are all explicit; (2) `supplier_reviews / listPendingReviews`, deferred because it has both Admin moderation and portal-reporting callers and is adjacent to the moderation lifecycle; (3) `supplier_submissions / listMySubmissions`, deferred because it participates in the Supplier dashboard alongside submission, duplicate, and access-credit workflows; and (4) `supplier_feedback / listMySupplierFeedback`, deferred because the same aggregate spans Buyer self-feedback, Supplier-profile display, and Admin moderation. `getPlatformSettings`, operational reporting, audit evidence, workspace reads, and AI intent remain non-finalists due to cross-aggregate/write coupling, cache or privacy scope, or non-datastore capability boundaries. No finalist requires a new identity source, Rule/Auth/index change, manifest change, migration/cutover, or Supabase capability; the selected operation is the clear bounded Low/Medium winner.
