...
try{
  ...
  $fixture=@("select public.seed_eligible_actor(1,'owner')","select public.seed_eligible_actor(2,'admin')",
    "select public.seed_eligible_actor(3,'owner')","select public.seed_claimant(n) from generate_series(101,109)n",
    "select public.seed_supplier(n) from generate_series(201,208)n",
  ...
  Q "update public.supplier_ownership_claims set submitted_at=statement_timestamp()-interval '720 hours'+interval '2 seconds',expires_at=statement_timestamp()+interval '2 seconds',updated_at=statement_timestamp()where id='$claim';" boundary-fixture|Out-Null
  ...
  Assert ($counts-eq'8/8/0/0') 'all races preserve expiry event cardinality zero audit and zero ownership'
  $sw.Stop();Write-Output ("Claim expire concurrency validation passed: {0} checks; eight true-session races; {1:n1}s elapsed."-f$checks.Count,$sw.Elapsed.TotalSeconds)
