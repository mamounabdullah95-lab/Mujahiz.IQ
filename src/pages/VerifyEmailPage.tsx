...
export function VerifyEmailPage() {
  ...
  const text = locale === "ar" ? {
    ...
    success: "تم تفعيل البريد بنجاح، وبدأت فترة الوصول المجانية.",
    ...
    success: "Your email is verified and the free access period has started.",
  ...
  async function resend() { if (cooldown) return; setBusy(true); setError(""); try { await sendVerification(); setMessage(text.sent); setCooldown(60); } catch { setError(locale === "ar" ? "تعذر إرسال الرسالة الآن. حاول بعد قليل." : "The email could not be sent. Try again shortly."); } finally { setBusy(false); } }
  async function refresh() { setBusy(true); setError(""); try { const verified = await refreshEmailVerification(); setMessage(verified ? text.success : text.waiting); if (verified && appUser) navigate(portalHome(appUser), { replace: true }); } catch { setError(locale === "ar" ? "تعذر تحديث حالة التفعيل." : "Verification status could not be refreshed."); } finally { setBusy(false); } }
