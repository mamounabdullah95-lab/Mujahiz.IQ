...
export function VerifyEmailPage() {
  ...
  const successMessage = appUser?.accountType === "buyer" ? text.buyerSuccess : text.supplierSuccess;
  ...
  async function refresh() { setBusy(true); setError(""); try { const verified = await refreshEmailVerification(); setMessage(verified ? successMessage : text.waiting); if (verified && appUser) navigate(portalHome(appUser), { replace: true }); } catch { setError(locale === "ar" ? "تعذر تحديث حالة التفعيل." : "Verification status could not be refreshed."); } finally { setBusy(false); } }
