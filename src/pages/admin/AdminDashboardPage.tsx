...
import { listPendingReviews, listSupplierFeedback, listSupplierSubmissions, listSuppliers, listUsers } from "../../services/firestore";

export function AdminDashboardPage() {
  const { t } = useTranslation();
  const [stats, setStats] = useState({ submissions: 0, reviews: 0, feedback: 0, pendingUsers: 0, suppliers: 0 });
  ...
  useEffect(() => {
    void Promise.all([
      ...
      listSuppliers(),
    ]).then(([submissions, reviews, feedback, users, suppliers]) => {
      setStats({
