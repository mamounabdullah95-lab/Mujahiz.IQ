-- Nineteenth local SQL slice: corrective supplier_claim.submit idempotency hotfix.
-- This migration adds no table, Claim command, mutation RLS policy, notification,
-- hosted capability, Firebase integration, real row, or data movement.

create function supplier_claim._canonicalize_submit_request_v1(
...
returns table (
  ...
  normalized_submitted_reason text,
  normalized_evidence_descriptors jsonb,
  key_digest bytea,
...
begin
  ...
  normalized_submitted_reason := v_reason;
  normalized_evidence_descriptors := v_evidence;
  return next;
end
$function$;

comment on function supplier_claim._canonicalize_submit_request_v1(text, uuid, text, text, jsonb, uuid) is
  'Private non-callable submit-v1 canonicalizer. It derives the trusted claimant and HMAC digests without returning the HMAC key, raw idempotency key, request body, or execution fence.';

alter function supplier_claim._canonicalize_submit_request_v1(text, uuid, text, text, jsonb, uuid)
  owner to postgres;

revoke all on function supplier_claim._canonicalize_submit_request_v1(text, uuid, text, text, jsonb, uuid)
  from public, anon, authenticated, service_role, mujahiz_claim_runtime;

create function supplier_claim.reserve_submit(
  p_idempotency_key text,
  p_supplier_profile_id uuid,
  p_submitted_reason text,
  p_evidence_schema_version text,
...
declare
  ...
  v_submit_event internal.domain_events%rowtype;
  v_new_reservation boolean := false;
  v_event_count integer := 0;
  v_claim_submit_event_count integer := 0;
begin
  select canonical.*
  into v_request
  from supplier_claim._canonicalize_submit_request_v1(
    p_idempotency_key,
    p_supplier_profile_id,
    p_submitted_reason,
    p_evidence_schema_version,
    p_evidence_descriptors,
    p_prior_claim_id
  ) as canonical;
  ...
  v_fence_digest := extensions.hmac(
    pg_catalog.convert_to('claim-submit-fence-v1|' || v_fence::text, 'UTF8'),
    pg_catalog.convert_to(v_hmac_key, 'UTF8'),
    'sha256'
  );
  ...
  if not v_new_reservation then
    ...
    if v_idempotency.principal_kind <> 'human_user'
       ...
       or v_idempotency.target_aggregate_id is distinct from v_request.slot_id
    ...
    if v_idempotency.status = 'completed' then
      ...
      if not found
         ...
         or v_replay_claim.submission_fingerprint <> pg_catalog.encode(v_idempotency.request_fingerprint, 'hex')
         or v_replay_claim.submission_fingerprint <> pg_catalog.encode(v_request.request_fingerprint, 'hex')
         ...
         or v_idempotency.completed_at <> v_replay_claim.submitted_at
         or v_idempotency.expires_at <> v_replay_claim.expires_at
      then
        raise exception using
          errcode = 'P5199',
          message = 'integrity_reconciliation_required';
      ...
      if not found
         or v_submit_event.event_type <> 'supplier_ownership.claim_submitted'
         or v_submit_event.event_schema_version <> 1
         or v_submit_event.aggregate_type <> 'supplier_ownership_claim'
         or v_submit_event.aggregate_id <> v_replay_claim.id
         or v_submit_event.aggregate_sequence <> 1
         or v_submit_event.producer_command_name <> 'supplier_claim.submit'
         or v_submit_event.producer_command_contract_version <> 1
         or v_submit_event.source_operation_identity is not null
         or v_submit_event.source_system_code <> 'mujahiz'
         or v_submit_event.source_stream_code is not null
         or v_submit_event.source_event_id is not null
         or v_submit_event.actor_kind <> 'human_user'
         or v_submit_event.actor_user_profile_id <> v_request.claimant_user_profile_id
         or v_submit_event.actor_source_code is not null
         or v_submit_event.environment_code <> 'local'
         or v_submit_event.producing_component_code <> 'supplier_claim_command'
         or v_submit_event.causation_event_id is not null
         or v_submit_event.occurred_at <> v_replay_claim.submitted_at
         or v_submit_event.persisted_at <> v_replay_claim.submitted_at
         or v_submit_event.is_historical
         or v_submit_event.fanout_suppressed
         or v_submit_event.migration_classification_code is not null
         or v_submit_event.payload <> pg_catalog.jsonb_build_object(
      ...
      select pg_catalog.count(*)
      into v_claim_submit_event_count
      from internal.domain_events as event_row
      where event_row.aggregate_type = 'supplier_ownership_claim'
        and event_row.aggregate_id = v_replay_claim.id
        and event_row.event_type = 'supplier_ownership.claim_submitted';

      if v_claim_submit_event_count <> 1 then
        raise exception using
          errcode = 'P5199',
          message = 'integrity_reconciliation_required';
      end if;
...
comment on function supplier_claim.reserve_submit(text, uuid, text, text, jsonb, uuid) is
  'Private trusted phase-1 submit reservation/replay/reclaim boundary. It commits no Claim/event, derives the claimant and fingerprints from trusted context, returns only a server-generated opaque execution fence or immutable replay result, and terminalizes exhausted attempt-10 leases for reconciliation.';

alter function supplier_claim.reserve_submit(text, uuid, text, text, jsonb, uuid)
  owner to postgres;

revoke all on function supplier_claim.reserve_submit(text, uuid, text, text, jsonb, uuid)
  from public, anon, authenticated, service_role, mujahiz_claim_runtime;
grant execute on function supplier_claim.reserve_submit(text, uuid, text, text, jsonb, uuid)
  to mujahiz_claim_runtime;

revoke all on function supplier_claim.submit(text, uuid, text, text, jsonb, uuid, uuid)
  from public, anon, authenticated, service_role, mujahiz_claim_runtime;
drop function supplier_claim.submit(text, uuid, text, text, jsonb, uuid, uuid);
...
begin
  ...
  v_fence_digest := extensions.hmac(
    pg_catalog.convert_to('claim-submit-fence-v1|' || p_execution_fence::text, 'UTF8'),
    pg_catalog.convert_to(v_hmac_key, 'UTF8'),
    'sha256'
  );
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    claim_security.claim_principal_lock_key_v1(v_request.claimant_user_profile_id)
  );
  perform pg_catalog.pg_advisory_xact_lock(
  ...
  where profile_row.id = v_request.claimant_user_profile_id
  for update;
  ...
  for v_provider_link in
    ...
    where provider_link_row.user_profile_id = v_request.claimant_user_profile_id
  ...
  for v_prior_claim in
    ...
    where claim_row.supplier_profile_id = p_supplier_profile_id
      and claim_row.claimant_user_profile_id = v_request.claimant_user_profile_id
  ...
  if p_prior_claim_id is not null then
    ...
    if not found
       or v_prior_claim.claimant_user_profile_id <> v_request.claimant_user_profile_id
       ...
       or (
         v_prior_claim.submitted_reason = v_request.normalized_submitted_reason
         and v_prior_claim.evidence_schema_version = 'claim_evidence_v1'
         and v_prior_claim.evidence_descriptors = v_request.normalized_evidence_descriptors
       )
    then
      raise exception using
        errcode = 'P5107',
  ...
  insert into public.supplier_ownership_claims (
    ...
    v_request.normalized_submitted_reason,
    'claimant_snapshot_v1',
    v_claimant_snapshot,
    'claim_submit_v1',
    pg_catalog.encode(v_request.request_fingerprint, 'hex'),
    'claim_evidence_v1',
    v_request.normalized_evidence_descriptors,
    p_prior_claim_id,
    v_command_now,
    v_command_now
  );
...
exception
  ...
  when unique_violation then
    ...
    if v_constraint_schema = 'public'
       and v_table_name = 'supplier_ownership_claims'
       and v_constraint_name = 'supplier_ownership_claims_one_active_pair_uidx'
    then
      raise exception using
        errcode = 'P5106',
        message = 'active_claim_exists';
...
comment on function supplier_claim.submit(text, uuid, text, text, jsonb, uuid, uuid, uuid) is
  'Local trusted supplier_claim.submit v1 phase-2 executor. It requires the exact committed phase-1 fence, re-derives the claimant and request binding, takes versioned principal/Supplier locks, inserts one submitted Claim and one claim_submitted event, completes the fenced result atomically, and writes no notification or ordinary-submit audit row.';

alter function supplier_claim.submit(text, uuid, text, text, jsonb, uuid, uuid, uuid)
  owner to postgres;

revoke all on function supplier_claim.submit(text, uuid, text, text, jsonb, uuid, uuid, uuid)
  from public, anon, authenticated, service_role, mujahiz_claim_runtime;
grant execute on function supplier_claim.submit(text, uuid, text, text, jsonb, uuid, uuid, uuid)
  to mujahiz_claim_runtime;
