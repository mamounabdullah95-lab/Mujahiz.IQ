...
select has_column('internal', 'audit_logs', 'environment_code', 'audit_logs records a stable environment identity');
select has_column('internal', 'audit_logs', 'source_system_code', 'audit_logs records a bounded source-system code');
select has_column('internal', 'audit_logs', 'producing_component_code', 'audit_logs records a bounded producing component or command');
select has_column('internal', 'audit_logs', 'source_operation_class', 'audit_logs records a bounded source-operation class');
select ok(exists (
  select 1
  from pg_catalog.pg_constraint
  where conrelid = 'internal.audit_logs'::regclass
    and conname = 'audit_logs_environment_code_ck'
    and pg_catalog.pg_get_constraintdef(oid) like '%environment_code = ANY%'
), 'environment identity is limited to approved stable environment codes');
select is(
  (
    select count(*)
    from pg_catalog.pg_constraint
    where conrelid = 'internal.audit_logs'::regclass
      and conname in (
        'audit_logs_source_system_code_ck',
        'audit_logs_producing_component_code_ck',
        'audit_logs_source_operation_class_ck'
      )
  ),
  3::bigint,
  'source-system, producing-component, and source-operation values have bounded code constraints'
);
select is(
...
select ok(col_description('internal.audit_logs'::regclass, (select attnum from pg_catalog.pg_attribute where attrelid = 'internal.audit_logs'::regclass and attname = 'idempotency_reference')) ilike '%no internal.idempotency_keys%', 'idempotency reference comment preserves table separation');
select ok(col_description('internal.audit_logs'::regclass, (select attnum from pg_catalog.pg_attribute where attrelid = 'internal.audit_logs'::regclass and attname = 'domain_event_reference')) ilike '%no internal.domain_events%', 'event reference comment preserves table separation');
select ok(col_description('internal.audit_logs'::regclass, (select attnum from pg_catalog.pg_attribute where attrelid = 'internal.audit_logs'::regclass and attname = 'environment_code')) ilike '%not a hostname%', 'environment comment excludes deployment-instance and host evidence');
...
with inserted as (
  insert into internal.audit_logs (
    ...
    target_entity_type, target_id, environment_code, source_system_code, producing_component_code,
    source_operation_class, outcome_class, result_code, reason_code, correlation_id,
    ...
    'local', 'audit_service', 'claim_decision_command', 'trusted_command',
...
select lives_ok($$ insert into internal.audit_logs (action_code, action_contract_version, action_class, actor_kind, actor_source_code, target_entity_type, target_external_reference, environment_code, source_system_code, producing_component_code, source_operation_class, outcome_class, result_code, reason_code, correlation_id, audit_schema_version, action_evidence_schema_version, authorization_policy_version, producer_contract_version, minimization_policy_version, retention_class) values ('migration.reconcile', 2, 'migration_reconciliation', 'migration_operator', 'migration_service', 'migration_batch', 'synthetic-batch-ref', 'local', 'migration_service', 'reconciliation_command', 'migration_reconciliation', 'succeeded', 'reconciled', 'approved_batch', gen_random_uuid(), 'audit_v1', 'migration_evidence_v1', 'migration_policy_v1', 'migration_service_v2', 'minimization_v1', 'migration_reconciliation') $$, 'trusted migration source without a human profile is structurally supported');
select throws_ok($$ insert into internal.audit_logs (action_code, action_contract_version, action_class, actor_kind, target_entity_type, target_id, environment_code, source_system_code, producing_component_code, source_operation_class, outcome_class, result_code, reason_code, correlation_id, audit_schema_version, action_evidence_schema_version, authorization_policy_version, producer_contract_version, minimization_policy_version, retention_class) values ('role.assign', 1, 'platform_privilege', 'human_user', 'user_profile', gen_random_uuid(), 'local', 'role_service', 'role_command', 'trusted_command', 'rejected', 'missing_actor', 'missing_actor', gen_random_uuid(), 'audit_v1', 'role_evidence_v1', 'role_policy_v1', 'role_command_v1', 'minimization_v1', 'privilege_security_authority') $$, null, 'human audit actor must use a provider-neutral profile');
select throws_ok($$ insert into internal.audit_logs (action_code, action_contract_version, action_class, actor_kind, actor_source_code, target_entity_type, target_id, target_external_reference, environment_code, source_system_code, producing_component_code, source_operation_class, outcome_class, result_code, reason_code, correlation_id, audit_schema_version, action_evidence_schema_version, authorization_policy_version, producer_contract_version, minimization_policy_version, retention_class) values ('role.assign', 1, 'platform_privilege', 'trusted_service', 'role_service', 'user_profile', gen_random_uuid(), 'raw-ref', 'local', 'role_service', 'role_command', 'trusted_command', 'failed', 'invalid_target', 'invalid_target', gen_random_uuid(), 'audit_v1', 'role_evidence_v1', 'role_policy_v1', 'role_command_v1', 'minimization_v1', 'privilege_security_authority') $$, null, 'target uses exactly one opaque identifier form');
select throws_ok($$ insert into internal.audit_logs (action_code, action_contract_version, action_class, actor_kind, actor_source_code, target_entity_type, target_id, environment_code, source_system_code, producing_component_code, source_operation_class, outcome_class, result_code, reason_code, correlation_id, audit_schema_version, action_evidence_schema_version, authorization_policy_version, producer_contract_version, minimization_policy_version, retention_class, predecessor_audit_log_id, correction_reason_code) values ('role.correct', 1, 'platform_privilege', 'trusted_service', 'role_service', 'user_profile', gen_random_uuid(), 'local', 'role_service', 'role_command', 'trusted_command', 'rejected', 'invalid_correction', 'invalid_correction', gen_random_uuid(), 'audit_v1', 'role_evidence_v1', 'role_policy_v1', 'role_command_v1', 'minimization_v1', 'privilege_security_authority', (select id from audit_log_test_ids where name = 'rejected_attempt'), 'synthetic_correction') $$, null, 'only corrected rows may link a predecessor audit record');
select lives_ok($$ insert into internal.audit_logs (action_code, action_contract_version, action_class, actor_kind, actor_source_code, target_entity_type, target_id, environment_code, source_system_code, producing_component_code, source_operation_class, outcome_class, result_code, reason_code, correlation_id, audit_schema_version, action_evidence_schema_version, authorization_policy_version, producer_contract_version, minimization_policy_version, retention_class, predecessor_audit_log_id, correction_reason_code) values ('role.correct', 1, 'platform_privilege', 'trusted_service', 'role_service', 'user_profile', gen_random_uuid(), 'local', 'role_service', 'role_command', 'trusted_command', 'corrected', 'corrected_record', 'approved_correction', gen_random_uuid(), 'audit_v1', 'role_evidence_v1', 'role_policy_v1', 'role_command_v1', 'minimization_v1', 'privilege_security_authority', (select id from audit_log_test_ids where name = 'rejected_attempt'), 'synthetic_correction') $$, 'corrected audit row retains a restrictive predecessor link');
select throws_ok($$ insert into internal.audit_logs (action_code, action_contract_version, action_class, actor_kind, actor_source_code, target_entity_type, target_id, environment_code, source_system_code, producing_component_code, source_operation_class, outcome_class, result_code, reason_code, safe_context_schema_version, safe_context, correlation_id, audit_schema_version, action_evidence_schema_version, authorization_policy_version, producer_contract_version, minimization_policy_version, retention_class) values ('role.assign', 1, 'platform_privilege', 'trusted_service', 'role_service', 'user_profile', gen_random_uuid(), 'local', 'role_service', 'role_command', 'trusted_command', 'failed', 'invalid_context', 'invalid_context', 'safe_context_v1', '[]'::jsonb, gen_random_uuid(), 'audit_v1', 'role_evidence_v1', 'role_policy_v1', 'role_command_v1', 'minimization_v1', 'privilege_security_authority') $$, null, 'safe context must be a bounded versioned JSON object');

select throws_ok($$ insert into internal.audit_logs (action_code, action_contract_version, action_class, actor_kind, actor_source_code, target_entity_type, target_id, environment_code, source_system_code, producing_component_code, source_operation_class, outcome_class, result_code, reason_code, correlation_id, audit_schema_version, action_evidence_schema_version, authorization_policy_version, producer_contract_version, minimization_policy_version, retention_class) values ('role.assign', 1, 'platform_privilege', 'trusted_service', 'role_service', 'user_profile', gen_random_uuid(), 'local_test', 'role_service', 'role_command', 'trusted_command', 'failed', 'invalid_environment', 'invalid_environment', gen_random_uuid(), 'audit_v1', 'role_evidence_v1', 'role_policy_v1', 'role_command_v1', 'minimization_v1', 'privilege_security_authority') $$, null, 'environment identity must use the approved bounded vocabulary');
select throws_ok($$ insert into internal.audit_logs (action_code, action_contract_version, action_class, actor_kind, actor_source_code, target_entity_type, target_id, environment_code, source_system_code, producing_component_code, source_operation_class, outcome_class, result_code, reason_code, correlation_id, audit_schema_version, action_evidence_schema_version, authorization_policy_version, producer_contract_version, minimization_policy_version, retention_class) values ('role.assign', 1, 'platform_privilege', 'trusted_service', 'role_service', 'user_profile', gen_random_uuid(), 'local', 'role_service', 'role/command', 'trusted_command', 'failed', 'invalid_component', 'invalid_component', gen_random_uuid(), 'audit_v1', 'role_evidence_v1', 'role_policy_v1', 'role_command_v1', 'minimization_v1', 'privilege_security_authority') $$, null, 'producing component must use a bounded non-secret code');
