...
select throws_ok($$ insert into public.platform_role_assignments (user_profile_id, role_code, assignment_source_type, assignment_reason_code, authorization_policy_version, evidence_reference, assigned_by_user_profile_id) values (gen_random_uuid(), 'reviewer', 'role_administration', 'synthetic_reviewer_rejection', 'platform_roles_v1', 'synthetic-evidence-reviewer', (select id from platform_role_test_ids where name = 'grantor')) $$, null, 'reviewer is rejected as a platform role');
...
select throws_ok($$ insert into public.platform_role_assignments (user_profile_id, role_code, assignment_status, valid_until, assignment_source_type, assignment_reason_code, authorization_policy_version, evidence_reference, assigned_by_user_profile_id) values (gen_random_uuid(), 'owner', 'active', statement_timestamp() + interval '1 day', 'role_administration', 'synthetic_terminal_active', 'platform_roles_v1', 'synthetic-evidence-terminal-active', (select id from platform_role_test_ids where name = 'grantor')) $$, null, 'active assignment cannot carry terminal provenance-free expiration state');
select throws_ok($$ insert into public.platform_role_assignments (user_profile_id, role_code, assignment_status, valid_from, valid_until, assignment_source_type, assignment_reason_code, authorization_policy_version, evidence_reference, assigned_by_user_profile_id) values (gen_random_uuid(), 'admin', 'revoked', statement_timestamp(), statement_timestamp() + interval '1 day', 'role_administration', 'synthetic_missing_terminal', 'platform_roles_v1', 'synthetic-evidence-missing-terminal', (select id from platform_role_test_ids where name = 'grantor')) $$, null, 'terminal lifecycle requires terminal provenance');
select throws_ok($$ insert into public.platform_role_assignments (user_profile_id, role_code, valid_from, valid_until, assignment_source_type, assignment_reason_code, authorization_policy_version, evidence_reference, assigned_by_user_profile_id) values (gen_random_uuid(), 'admin', timestamptz '2026-03-01 00:00:00+00', timestamptz '2026-03-01 00:00:00+00', 'role_administration', 'synthetic_bad_interval', 'platform_roles_v1', 'synthetic-evidence-bad-interval', (select id from platform_role_test_ids where name = 'grantor')) $$, null, 'validity interval must be half-open and non-empty');
select throws_ok($$ insert into public.platform_role_assignments (user_profile_id, role_code, assignment_source_type, assignment_reason_code, authorization_policy_version, evidence_reference, assignment_system_source) values (gen_random_uuid(), 'admin', 'role_administration', 'synthetic_wrong_provenance', 'platform_roles_v1', 'synthetic-evidence-wrong-provenance', 'synthetic_system') $$, null, 'ordinary assignments require a human grantor rather than a system source');
select throws_ok($$ insert into public.platform_role_assignments (user_profile_id, role_code, assignment_source_type, assignment_reason_code, authorization_policy_version, evidence_reference, assignment_system_source, reviewed_by_user_profile_id) values (gen_random_uuid(), 'owner', 'bootstrap_manifest', 'synthetic_bootstrap', 'platform_roles_v1', 'synthetic-evidence-bootstrap', 'synthetic_bootstrap', (select id from platform_role_test_ids where name = 'grantor')) $$, null, 'bootstrap reviewer must reference a valid profile');
